# Rocky Pet — a desktop companion for Omarchy

A pixel-art pet that lives on your Hyprland desktop, **delivers your
notifications**, mirrors what your **AI coding agents** (Claude Code, Codex) are
doing, reflects **machine health**, and doubles as the **screensaver**. It runs
inside the Omarchy shell (Quickshell) — no extra daemon, no Electron.
...
| **Tired** | Package > 85 °C, thermal throttling, or load > 1 per core for 60 s | Sweating, slumped |
| **Working** | An agent is in a turn (`UserPromptSubmit` → `Stop`) | Types on a tiny laptop |
...
### Notifications, delivered in person
...
watches the service's per-toast state files and shows the newest one in a
speech bubble with the app, summary and body, "+N more" when several are
queued, and a red frame for critical urgency. **Left click** runs the
notification's action, **right click** dismisses it. Each event plays a short
sound (normal, critical, "needs you", "done") with a cooldown so a hook and its
own desktop notification never ring twice.
...
starts, the shell switches to a black overlay where the same pet strolls,
pauses and naps across the whole screen (changing height after each nap, so
OLED panels never see the same pixels twice).
...
```bash
git clone https://github.com/jlopezlira/rocky-pet ~/Projects/rocky-pet
cd ~/Projects/rocky-pet
...
```bash
omarchy-shell pet status | jq
```

### Other pets

Any built-in Codex pet: `./install.sh --pet dewey` (ids: `codex dewey fireball
rocky seedy stacky bsod null-signal`). A community pet folder containing
`pet.json` + `spritesheet.webp`: `pet-fetch <id> /path/to/folder`, then point
`Service.qml` at it (the installer does this for `--pet`).

## Configuration

`~/.config/omarchy/pet.json` — hot-reloaded, no restart needed:
...
```bash
omarchy-shell pet status              # JSON: state, agents, notes, health, positions
omarchy-shell pet screensaverToggle   # try the screensaver mode right now
...
```
Claude Code / Codex hooks ──▶ pet-agent-state ──▶ ~/.local/state/omarchy/pet/agents/<agent>-<session>.json
...
pet-health (every 10 s, 60 s when asleep) ────▶ JSON on stdout
                                                        │
...
- **Cursor facing**: layer surfaces only see the pointer over the sprite, so the
  global cursor is polled from Hyprland (`hyprctl cursorpos`, 5 Hz, paused when
  asleep or hidden).
...
```
plugin/jlopezlira.pet/      the Quickshell service plugin (manifest.json, Service.qml)
bin/pet-health              machine health snapshot (JSON), root-free
bin/pet-agent-state         called by the agent hooks; one file per agent session
bin/pet-fetch               downloads a built-in Codex pet or installs a community one
...
config/pet.json             default settings
