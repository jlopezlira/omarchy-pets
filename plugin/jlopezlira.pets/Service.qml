...
Item {
  ...
  IpcHandler {
    target: "pets"
    function status(): string { return JSON.stringify({ pet: root.pet ? root.pet.id : null, state: root.petState, thought: root.thought, asleep: root.asleep, tired: root.tired, weak: root.weak, cursorX: root.cursorX, notes: root.notes.length, agents: root.agents, active: root.activeAgents, hungry: root.hungryAgents, sounds: root.soundsOn, lastSound: root.lastSound, positions: root.positions, health: root.health }) }
